...
Class Mongo_db{
	...
	*/
    ...
    /**
     * 扩展mongodb的功能，实现数据的替换
     * @param string $collection
     * @param array $new_obj 准备覆盖的新的数据
     * @return bool 是否修改成功
     */
    public function replace($collection = "",$new_obj = array()){
        if (empty($collection))
        {
            show_error("No Mongo collection selected for replace", 500);
        }

        try
        {
            $this->db->{$collection}->update($this->wheres, $new_obj);
            $this->_clear();
            return (TRUE);
        }
        catch (MongoCursorException $e)
        {
            if(isset($this->debug) == TRUE && $this->debug == TRUE)
            {
                show_error("Replace of data into MongoDB failed: {$e->getMessage()}", 500);
            }
            else
            {
                show_error("Replace of data into MongoDB failed", 500);
            }
        }
    }
	/**
